...
public class SentinelShardPool extends Pool<ShardedJedis> {
	...
	private void initPool(List<HostAndPort> masters) {
		if (!equals(currentHostMasters, masters)) {
			...
			initPool(redisPoolConfig, new ShardedJedisFactory(shardMasters,
	...
	private List<HostAndPort> initSentinels(String sentinels,
		...
		if (masters != null && masters.length() > 0) {
			String[] masterNames = masters.split(",");
			for (String masterName : masterNames) {
				...
				while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
					if (sentinels != null && sentinels.length() > 0) {
						String[] sentinelAddrs = sentinels.split(",");
						for (String sentinel : sentinelAddrs) {
							...
							try {
								Jedis jedis = new Jedis(hap.getHost(),
										hap.getPort());
								...
								if (master == null) {
									...
									if (hostAndPort != null
										...
										log.fine("Found Redis master at "
												+ master);
				...
				if (!fetched && sentinelRetry >= MAX_RETRY_SENTINEL) {
					log.severe("All sentinels down and try "
							+ MAX_RETRY_SENTINEL + " times, Abort.");
			...
			if (masterNames.length != 0
					&& masterNames.length == shardMasters.size()) {
				...
				if (sentinels != null && sentinels.length() > 0) {
					...
					for (String sentinel : sentinelAddrs) {
						...
						MasterListener masterListener = new MasterListener(
								Arrays.asList(masterNames), hap.getHost(),
								hap.getPort());
